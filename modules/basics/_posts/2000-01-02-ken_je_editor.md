...
Een overzicht van mogelijke Views vind je in het menu _'View'_. Onderaan de lijst met beschikbare views vind je ook nog een uitklapmenu _'Other Windows'_ met nog meer mogelijkheden.

### Solution Explorer

Wanneer je een project opent, dan toont Visual Studio de _Solution Explorer_. Een __Solution__ is een verzameling van code, afbeeldingen enzovoort die je samen wil gebruiken. Voorlopig zullen we slechts een project per Solution nodig hebben, maar dat wordt later meer.

De _Solution Explorer_ toont de bestanden die bij je code horen op een gestructureerde manier.
...
Om van je code een programma te maken, gebruik je de `Start` knop, of je drukt op `F5`. Heb je geen fouten gemaakt, dan start je programma. Staan er fouten in je code, dan toont Visual Studio de view _Error List_. Deze toont je een overzicht van de fouten in de code.

<div class="note protip">
<p>Los steeds de eerste foutmelding eerst op. Dikwijls genereert een enkele fout meer dan één melding.</p>
</div>

Via een double-click op de foutmelding brengt Visual Studio je naar het bestand en de regel waar het probleem zit. Dit helpt je om snel te vinden waar het mis gaat (al is dat in het voorbeeld hieronder zo ook wel duidelijk).
...
Via _'View > Other Windows'_ vind je ook _'C# Interactive'_. Dit is een console interface waarin je code kan uitproberen. C# Interactive is niet bedoeld om echt programma's in te schrijven, maar kan van pas komen wanneer je snel iets wil testen. Naast kennis van C# heb je ook enkele commando's nodig:
...
<div class="header2" markdown = "1">## Code Editor
</div>
...
En de editor helpt je nog meer! Code die niet helemaal OK is, wordt aangeduid met _Squiggly Lines_ _(gegolfde lijnen, maar het klinkt leuker in het Engels)_.
...
- grijze puntjes: Dit is een stijlfout. Zo begint de functienaam hieronder met een kleine letter. Dat is niet fout, maar er is een algemene consensus om public functies te laten beginnen met een hoofdletter. Zo kan je later altijd eenvoudig zien dat het om een public functie gaat.
...
```csharp
// Een enkele commentaarlijn wordt voorafgegaan door een dubbele slash.
...
namespace MijnEersteProgramma
{
  internal class Program
  {
     ...
     * Dit is handig als je meerdere lijnen in commentaar wil zetten.
     We hoeven niet elke regel met een asterisk te beginnen.
     */
    static void Main(string[] args)
    {
        Console.WriteLine("Hello, World!");
